...
### `(co.)peak_expansion_gain_mono_db`

Mono dynamic range expander gain computer with dB output. This is
the per-channel core of `peak_expansion_gain_N_chan_db`.

#### Usage

```
_ : peak_expansion_gain_mono_db(maxHold,strength,thresh,range,attack,hold,release,knee,prePost) : _
```
